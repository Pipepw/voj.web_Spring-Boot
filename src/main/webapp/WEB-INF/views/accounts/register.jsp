...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="${BaseUrl}/css/accounts/register.css?v=${version}" />
    <!-- JavaScript -->
...
<body>
    ...
    <div id="content">
        ...
        <c:otherwise>
            <form id="register-form" method="POST" onsubmit="onSubmit(); return false;">
                ...
                <p class="row-fluid">
                    <label for="trueName"><spring:message code="voj.accounts.register.trueName" text="TrueName" /></label>
                    <input id="trueName" name="true_name" class="span12" type="text" maxlength="16" />
                </p>
                <p class="row-fluid">
                    <label for="stuId"><spring:message code="voj.accounts.register.stuId" text="Grade" /></label>
                    <input id="stuId" name="stuId" class="span12" type="text" maxlength="16" />
                </p>
                <p class="row-fluid">
    ...
    <script type="text/javascript">
        function onSubmit() {
            ...
            return doRegisterAction(username, password, email, trueName, stuId, languagePreference, csrfToken);
        };
    </script>
    <script type="text/javascript">
        function doRegisterAction(username, password, email, trueName, stuId, languagePreference, csrfToken) {
            var postData = {
                'username': username,
                'password': password,
                'email': email,
        ...
        function processRegisterResult(result) {
            if ( result['isSuccessful'] ) {
                ...
                if( result['isTrueNameEmpty']){
                    errorMessage += '<spring:message code="voj.accounts.register.trueName-empty" text="You can&apos;t leave True Name empty." /><br>';
                }
                if ( result['isStuIdEmpty'] ) {
                    errorMessage += '<spring:message code="voj.accounts.register.stuId-empty" text="You can&apos;t leave Student Number empty." /><br>';
                }else if ( !result['isStuIdLegal'] ) {
                    errorMessage += '<spring:message code="voj.accounts.register.stuId-illegal" text="The Student Number seems invalid." /><br>';
                }
