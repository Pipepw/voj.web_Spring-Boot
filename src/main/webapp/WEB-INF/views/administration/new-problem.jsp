...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="${BaseUrl}/css/administration/new-problem.css?v=${version}" />
    <!-- JavaScript -->
...
<body>
    <div id="wrapper">
        ...
        <div id="container">
            ...
            <div id="content">
                ...
                <form method="post" onSubmit="onImport(); return false;" enctype="multipart/form-data">
                    选择一个文件:
                    <input type="file" id="problem_file" name="problem_file" />
                    <br/><br/>
                    <div style="display: flex;">
                        <button id="problem-import" class="btn btn-primary" type="submit"><spring:message code="voj.administration.new-problem.import-problem" text="Import" /></button>
                        <div id="import_text"></div>
                    </div>
                </form>
    ...
    <script type="text/javascript">
        function onImport(){
            // var files = $('#problem_file').prop("file");
            console.log($('#problem_file')[0]);
            var data = new FormData;
            data.append('problem_file',$('#problem_file')[0].files[0]);
            // data.append('problem_file',file[0]);
            console.log(data.get('problem_file'));
            $.ajax({
                type: 'POST',
                url: '<c:url value="/administration/importProblem" />',
                data: data,
                cache: false,
                processData: false,
                contentType: false,
                beforeSend: function () {
                    $("#import_text").text("导入中");
                },
                success: function (result) {
                    return processCreateProblemResult(result);
                },
                complete: function () {
                    $("#import_text").text("导入成功");
                }
            })
        }
    </script>
